...
void Perceptron::gradientDescent() // NOTE: градиентСпуск (видимо собственно обучение)
{
    for (int it = 0; it <
                 //    50   // видимо просто 50 раз (умолчание) проходит отображая cost
          100
         ; ++it) {
        resetDerivatives();
...
void Perceptron::setInput(RoundedDouble* arr, int length) // массив arr это видимо входная картинка
{
    if(layer.isEmpty()) {
...
const RoundedDouble* Perceptron::getOutput()
{
    ...
    return layer.last()->getValues(); // видимо выходной массив с 10-ю классами
}
